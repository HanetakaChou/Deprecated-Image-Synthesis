...
struct face
{
	...
	int v2;
	int v3;
	face(double v1i, double v2i, double v3i)
	{
		v1 = v1i;
		v2 = v2i;
		v3 = v3i;
...
bool LoadMayaHair(char *directory, bool b_shortHair)
{
	...
	for (int s = 0; s < int(HairStrands.size()); s++)
	{
		for (int i = 0; i < int(HairStrands.at(s).size()) - 1; i++)
		{
			...
			g_coordinateFrames[CFNum * 12 + 0] = x.x;
			g_coordinateFrames[CFNum * 12 + 1] = x.y;
			g_coordinateFrames[CFNum * 12 + 2] = x.z;
			g_coordinateFrames[CFNum * 12 + 3] = 0;
			g_coordinateFrames[CFNum * 12 + 4] = y.x;
			g_coordinateFrames[CFNum * 12 + 5] = y.y;
			g_coordinateFrames[CFNum * 12 + 6] = y.z;
			g_coordinateFrames[CFNum * 12 + 7] = 0;
			g_coordinateFrames[CFNum * 12 + 8] = z.x;
			g_coordinateFrames[CFNum * 12 + 9] = z.y;
			g_coordinateFrames[CFNum * 12 + 10] = z.z;
			g_coordinateFrames[CFNum * 12 + 11] = 0;
			CFNum++;
		}
