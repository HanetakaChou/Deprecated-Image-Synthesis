...
HRESULT InitApp()
{
	...
	if (g_renderMStrands && g_renderSStrands)
		g_InterpolateModel = HYBRID;
	else if (g_renderMStrands)
		g_InterpolateModel = MULTI_HYBRID;
	else if (g_renderSStrands)
		g_InterpolateModel = SINGLESTRAND;
	else
		g_InterpolateModel = NO_HAIR;
...
HRESULT CreateForceVB(ID3D11Device *pd3dDevice, ID3D11DeviceContext *pd3dContext)
{
	...
	for (int s = 0; s < g_NumMasterStrands; ++s)
	{
		...
		int location = x * 4 + y * texDesc.Width * 4;
...
HRESULT saveHair()
{
	...
	for (int i = 0; i < g_NumMasterStrandControlVertices; i++)
		outfile << '(' << g_MasterStrandControlVertices[i].Position.x << ',' << g_MasterStrandControlVertices[i].Position.y << ',' << g_MasterStrandControlVertices[i].Position.z << ')';
	outfile << endl;
