...
#include "../../dxbc/FurSampleSkinningRenderingPixelShader_bytecode.inl"
...
HRESULT CALLBACK OnD3D11CreateDevice(ID3D11Device *device, const DXGI_SURFACE_DESC *backBufferSurfaceDesc, void *userContext)
{
	...
	hr = FurSample_CreatePixelShader(device, FurSampleSkinningRenderingPixelShader_bytecode, sizeof(FurSampleSkinningRenderingPixelShader_bytecode), &g_customHairWorksShader);
	if (FAILED(hr))
		return hr;
...
void CALLBACK OnD3D11FrameRender(ID3D11Device *device, ID3D11DeviceContext *context, double time, float elapsedTime, void *userContext)
{
	...
	{
#ifndef NDEBUG
		g_d3dUserDefinedAnnotation->BeginEvent(L"Render Meshes");
#endif
