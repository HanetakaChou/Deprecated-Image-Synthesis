...
struct GFSDK_HairAssetDescriptor
{
	...
	GFSDK_HairAssetDescriptor::GFSDK_HairAssetDescriptor() : m_NumGuideHairs(0),
															 m_NumVertices(0),
															 m_pVertices(0),
															 m_pEndIndices(0),
...
struct GFSDK_HairAssetCopySettings
{
	...
	GFSDK_HairAssetCopySettings() : m_copyAll(true),
									m_copyGroom(true),
...
struct GFSDK_HairInstanceDescriptor
{
...

	{
		...
		m_tipColor.z = 1.0f;
		m_tipColor.w = 1.0f;
		m_specularColor.x = 1.0f;
		m_specularColor.y = 1.0f;
		m_specularColor.z = 1.0f;
		m_specularColor.w = 1.0f;

#define MAKE_IDENTITY(M) \
	M._11 = 1.0f;        \
	M._12 = 0.0f;        \
	M._13 = 0.0f;        \
	M._14 = 0.0f;        \
...
struct GFSDK_HairConversionSettings
{
	...
	gfsdk_F32 m_targetSceneUnit;					   //!< scene unit (in centimeters) for game engine.  0 means unknown (use source unit)
													   ...
													   // Decimeter: 10.0
public:
	GFSDK_HairConversionSettings() : m_targetUpAxisHint(GFSDK_HAIR_UP_AXIS_UNKNOWN),
...
public:
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES CopyAsset(
		...
		GFSDK_HairAssetCopySettings settings = GFSDK_HairAssetCopySettings()) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES ClearShaderCache() = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES AddToShaderCache(
		const GFSDK_HairShaderCacheSettings &settings) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES SaveShaderCacheToMemory(
		void **ppMemoryBuffer,
		size_t &memoryBufferSizeBytes) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES InitRenderResources(
		...
		ID3D11DeviceContext *pd3dContext = 0) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES CreateHairInstance(
		const GFSDK_HairAssetID hairAssetID,
		GFSDK_HairInstanceID *newInstanceID) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES FreeHairInstance(
		const GFSDK_HairInstanceID hairInstanceID) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES SetTextureSRV(
		...
		ID3D11ShaderResourceView *pResource) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES GetTextureSRV(
		const GFSDK_HairInstanceID hairInstanceID,
		const GFSDK_HAIR_TEXTURE_TYPE textureType,
		ID3D11ShaderResourceView **ppResource) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES GetShaderResources(
		const GFSDK_HairInstanceID hairInstanceID,
		ID3D11ShaderResourceView **ppResources) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES StepSimulation(
		...
		const gfsdk_float4x4 *worldReference = 0) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES PrepareShaderConstantBuffer(
		const GFSDK_HairInstanceID hairInstanceID,
		GFSDK_HairShaderConstantBuffer *pConstantBuffer) = 0;
	...
	*/
	virtual GFSDK_HAIR_RETURNCODES RenderHairs(
		const GFSDK_HairInstanceID hairInstanceID,
		const GFSDK_HairShaderSettings *pShaderSettings = 0) = 0;
	...
	int debugMode = 0)
{
	...
	if (hairDLLModule)
	{
		...
		if (!pCreateProc)
		{
			...
			hairDLLModule = NULL;
			if (pCustomLogHandler)
				pCustomLogHandler->Log(GFSDK_HAIR_LOG_ERROR, "HairWorks dll file not found", 0, 0);
		}
		else
