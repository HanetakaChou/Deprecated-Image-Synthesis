	...
	VkDeviceSize vulkan_upload_ring_buffer_size, VkBuffer vulkan_upload_ring_buffer)
{
	...
	this->create_render_pass_and_pipeline(vulkan_device, pfn_get_device_proc_addr, VK_FORMAT_D32_SFLOAT, VK_FORMAT_R8G8B8A8_UNORM, vulkan_allocation_callbacks);
...
void Demo::create_render_pass_and_pipeline(VkDevice vulkan_device, PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkFormat vulkan_depth_format, VkFormat vulkan_swapchain_image_format, VkAllocationCallbacks *vulkan_allocation_callbacks)
{
...
{
	if (vulkan_depth_format != this->m_vulkan_depth_format || vulkan_swapchain_image_format != this->m_vulkan_swapchain_image_format)
	{
		assert(VK_NULL_HANDLE != this->m_vulkan_render_pass);
		...
		pfn_destroy_render_pass(vulkan_device, this->m_vulkan_render_pass, vulkan_allocation_callbacks);

		assert(VK_NULL_HANDLE != this->m_vulkan_forward_shading_pipeline);
		...
		pfn_destroy_pipeline(vulkan_device, this->m_vulkan_forward_shading_pipeline, vulkan_allocation_callbacks);

		this->create_render_pass_and_pipeline(vulkan_device, pfn_get_device_proc_addr, vulkan_depth_format, vulkan_swapchain_image_format, vulkan_allocation_callbacks);

		this->m_vulkan_depth_format = vulkan_depth_format;
		this->m_vulkan_swapchain_image_format = vulkan_swapchain_image_format;
	}
