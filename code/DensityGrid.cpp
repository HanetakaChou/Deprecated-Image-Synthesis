...
HairGrid::HairGrid(ID3D11Device *pd3dDevice, ID3D11DeviceContext *pd3dContext)
    : renderQuadBuffer(NULL),
      renderQuadBufferForGrid(NULL),
      m_cols(0),
      m_rows(0),
      m_fluidGridWidth(0),
      m_fluidGridHeight(0),
...
void HairGrid::ReloadShader()
{
    HairEffect_SetDensityGrid(float(this->m_Width), float(this->m_Height), float(this->m_totalDepth), g_Width / this->m_cols, g_Height / this->m_rows);
    HairEffect_SetFluidObstacleTexture(this->m_fluidGridWidth, this->m_fluidGridHeight, this->m_fluidGridDepth);
}

HRESULT HairGrid::Initialize(int width, int height, int depth)
{
    HRESULT hr;
    ...
    V_RETURN(CreateRTTextureAsShaderResource(RENDER_TARGET_DENSITY_DEMUX, "Texture_density_Demux", &SRVDesc));

    SRVDesc.Texture2DArray.ArraySize = m_totalDepth;
    SRVDesc.Format = RenderTargetFormats[RENDER_TARGET_VOXELIZED_OBSTACLES];
    V_RETURN(CreateRTTextureAsShaderResource(RENDER_TARGET_VOXELIZED_OBSTACLES, "Texture_Voxelized_Obstacles", &SRVDesc));

    SRVDesc.Texture2DArray.ArraySize = m_totalDepth;
    SRVDesc.Format = RenderTargetFormats[RENDER_TARGET_DENSITY_BLUR_TEMP];
    V_RETURN(CreateRTTextureAsShaderResource(RENDER_TARGET_DENSITY_BLUR_TEMP, "Texture_density_Blur_Temp", &SRVDesc));

    SRVDesc.Texture2DArray.ArraySize = m_totalDepth;
    SRVDesc.Format = RenderTargetFormats[RENDER_TARGET_DENSITY_BLUR];
    V_RETURN(CreateRTTextureAsShaderResource(RENDER_TARGET_DENSITY_BLUR, "Texture_density_Blur", &SRVDesc));

    SRVDesc.Texture2DArray.ArraySize = m_Depth;
    SRVDesc.Format = RenderTargetFormats[RENDER_TARGET_DENSITY];
    V_RETURN(CreateRTTextureAsShaderResource(RENDER_TARGET_DENSITY, "Texture_density", &SRVDesc));
    ...
    V_RETURN(m_pD3DDevice->CreateInputLayout(layoutDesc, numElements, Hair_VS_GRID_bytecode, sizeof(Hair_VS_GRID_bytecode), &quad_layout));
...
HRESULT HairGrid::CreateRTTextureAsShaderResource(RENDER_TARGET rtIndex, LPCSTR shaderTextureName, D3D11_SHADER_RESOURCE_VIEW_DESC *SRVDesc)
{
    HRESULT hr;
...
HRESULT HairGrid::AddHairDensity(DirectX::XMFLOAT4X4 &objToVolumeXForm, HRESULT (*DrawHair)(DirectX::XMFLOAT4X4 &, ID3D11Device *, ID3D11DeviceContext *, float, float, int))
{
    HRESULT hr = S_OK;
    ...
    for (int z = 0; z < m_Depth; z += 8)
    {
        ...
        DirectX::XMMATRIX proj = DirectX::XMMatrixOrthographicOffCenterLH(-0.5, 0.5, -0.5, 0.5, ((float)z) / m_Depth - 0.5f, (float)(z + 8) / m_Depth - 0.5f);

        DirectX::XMFLOAT4X4 worldViewProj;
        DirectX::XMStoreFloat4x4(&worldViewProj, DirectX::XMMatrixMultiply(DirectX::XMLoadFloat4x4(&objToVolumeXForm), proj));
...
HRESULT HairGrid::demultiplexTexture(ID3D11DeviceContext *context)
{
    HRESULT hr = S_OK;
    ...
    context->VSSetShaderResources(RENDER_TARGET_DENSITY, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY]);
    context->HSSetShaderResources(RENDER_TARGET_DENSITY, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY]);
    context->DSSetShaderResources(RENDER_TARGET_DENSITY, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY]);
    context->GSSetShaderResources(RENDER_TARGET_DENSITY, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY]);
    context->PSSetShaderResources(RENDER_TARGET_DENSITY, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY]);

    context->VSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->HSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->DSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->GSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->PSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    ...
    for (int i = 0; i < m_totalDepth; i++)
    {
        context->ClearRenderTargetView(pDensityDemuxRenderTargetViews[i], color);
        context->OMSetRenderTargets(1, &(pDensityDemuxRenderTargetViews[i]), NULL);
        HairEffect_SetGridZIndex(i);
        HairEffect_Apply_TextureDemux(context);

        // render the quad
        context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        context->IASetInputLayout(quad_layout);
        context->IASetVertexBuffers(0, 1, &renderQuadBufferForGrid, stride, offset);
        context->Draw(m_numVerticesRenderQuadsForGrid, 0);
    }

    ID3D11ShaderResourceView *const NULLSRV = NULL;
    context->VSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->HSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->DSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->GSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->PSSetShaderResources(RENDER_TARGET_VOXELIZED_OBSTACLES, 1, &NULLSRV);

    context->VSSetShaderResources(RENDER_TARGET_DENSITY, 1, &NULLSRV);
    context->HSSetShaderResources(RENDER_TARGET_DENSITY, 1, &NULLSRV);
    context->DSSetShaderResources(RENDER_TARGET_DENSITY, 1, &NULLSRV);
    context->GSSetShaderResources(RENDER_TARGET_DENSITY, 1, &NULLSRV);
    context->PSSetShaderResources(RENDER_TARGET_DENSITY, 1, &NULLSRV);
...
HRESULT HairGrid::voxelizeObstacles(ID3D11DeviceContext *context)
{
    HRESULT hr = S_OK;
    ...
    for (int i = 0; i < m_totalDepth; i++)
    {
        context->ClearRenderTargetView(pVoxelizedObstacleRenderTargetViews[i], color);
        context->OMSetRenderTargets(1, &(pVoxelizedObstacleRenderTargetViews[i]), NULL);
        HairEffect_SetGridZIndex(i);
        HairEffect_Apply_VoxelizeObstacles(context);
...
void HairGrid::setDemuxTexture(ID3D11DeviceContext *context)
{
    context->VSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_DEMUX]);
    context->HSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_DEMUX]);
    context->DSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_DEMUX]);
    context->GSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_DEMUX]);
    context->PSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_DEMUX]);
}

void HairGrid::setObstacleVoxelizedTexture(ID3D11DeviceContext *context)
{
    context->VSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->HSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->DSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->GSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
    context->PSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &pRenderTargetShaderViews[RENDER_TARGET_VOXELIZED_OBSTACLES]);
}

void HairGrid::setBlurTexture(ID3D11DeviceContext *context)
{
    context->VSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_BLUR]);
    context->HSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_BLUR]);
    context->DSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_BLUR]);
    context->GSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_BLUR]);
    context->PSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &pRenderTargetShaderViews[RENDER_TARGET_DENSITY_BLUR]);
}

void HairGrid::unsetDemuxTexture(ID3D11DeviceContext *context)
{
    ID3D11ShaderResourceView *const NULLSRV = NULL;

    context->VSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &NULLSRV);
    context->HSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &NULLSRV);
    context->DSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &NULLSRV);
    context->GSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &NULLSRV);
    context->PSSetShaderResources(SLOT_TEXTURE_DENSITY_DEMUX, 1, &NULLSRV);
}

void HairGrid::unsetObstacleVoxelizedTexture(ID3D11DeviceContext *context)
{
    ID3D11ShaderResourceView *const NULLSRV = NULL;

    context->VSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->HSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->DSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->GSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &NULLSRV);
    context->PSSetShaderResources(SLOT_TEXTURE_VOXELIZED_OBSTACLES, 1, &NULLSRV);
}

void HairGrid::unsetBlurTexture(ID3D11DeviceContext *context)
{
    ID3D11ShaderResourceView *const NULLSRV = NULL;

    context->VSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &NULLSRV);
    context->HSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &NULLSRV);
    context->DSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &NULLSRV);
    context->GSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &NULLSRV);
    context->PSSetShaderResources(SLOT_TEXTURE_DENSITY_BLUR, 1, &NULLSRV);
}

HRESULT HairGrid::demultiplexTextureTo3DFluidObstacles(ID3D11DeviceContext *context)
{
    HRESULT hr = S_OK;
    ...
    context->ClearRenderTargetView(m_pRenderTargetViewFluidObstacles, clearValue);
    ...
    HairEffect_Apply_DemuxTo3DFluidObstacles(context);
...
void HairGrid::InitScreenSlice(GRID_TEXTURE_DISPLAY_STRUCT **vertices, int z, int &index)
{
    ...
    tempVertex1.Pos = DirectX::XMFLOAT3(left, bottom, 0.0f);
    tempVertex1.Tex = DirectX::XMFLOAT3(0, 0, float(z));

    tempVertex2.Pos = DirectX::XMFLOAT3(right, bottom, 0.0f);
    tempVertex2.Tex = DirectX::XMFLOAT3(w, 0, float(z));

    tempVertex3.Pos = DirectX::XMFLOAT3(right, top, 0.0f);
    tempVertex3.Tex = DirectX::XMFLOAT3(w, h, float(z));

    tempVertex4.Pos = DirectX::XMFLOAT3(left, top, 0.0f);
...
void HairGrid::InitGridSlice(GRID_TEXTURE_DISPLAY_STRUCT **vertices)
{
    ...
    tempVertex1.Pos = DirectX::XMFLOAT3(left, bottom, 0.0f);
    tempVertex1.Tex = DirectX::XMFLOAT3(0, 0, 0);

    tempVertex2.Pos = DirectX::XMFLOAT3(right, bottom, 0.0f);
    tempVertex2.Tex = DirectX::XMFLOAT3(w, 0, 0);

    tempVertex3.Pos = DirectX::XMFLOAT3(right, top, 0.0f);
    tempVertex3.Tex = DirectX::XMFLOAT3(w, h, 0);

    tempVertex4.Pos = DirectX::XMFLOAT3(left, top, 0.0f);
    tempVertex4.Tex = DirectX::XMFLOAT3(0, h, 0);
...
HRESULT HairGrid::setFluidObstacleTexture(ID3D11Texture3D *texture, DXGI_FORMAT format, int texWidth, int texHeight, int texDepth)
{
    ...
    this->m_fluidGridWidth = texWidth;
    this->m_fluidGridHeight = texHeight;
    this->m_fluidGridDepth = texDepth;
