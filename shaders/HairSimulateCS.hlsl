...
float4 addForcesAndIntegrate(float4 position, float4 oldPosition, float4 force, int vertexID)
{
    ...
    outputPos.xyz = position.xyz + velocity.xyz + force.xyz * TimeStep * TimeStep;
...
[numthreads(64, 1, 1)] void UpdateParticlesSimulate(uint threadId
                                                    : SV_GroupIndex,
                                                      uint3 groupId
                                                    : SV_GroupID,
                                                    ...
                                                    : SV_DispatchThreadID)
{
    ...
    if (groupId.x > 0)
        readStart = g_SimulationStrandOffsets.Load(groupId - 1);
    ...
    {
        float4 prevPos;
        if (g_restoreToDefault)
            sharedPos[threadId] = prevPos = g_OriginalMasterStrand.Load(threadReadIndex);
        else if (g_integrate)
            prevPos = previousParticlePositions[threadReadIndex];
        else
            prevPos = sharedPos[threadId];
