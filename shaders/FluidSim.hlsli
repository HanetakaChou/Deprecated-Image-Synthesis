...
[maxvertexcount(3)] void GS_ARRAY(triangle VS_OUTPUT_FLUIDSIM In[3], inout TriangleStream<GS_OUTPUT_FLUIDSIM> triStream)
{
    ...
    [maxvertexcount(2)] void GS_ARRAY_LINE(line VS_OUTPUT_FLUIDSIM In[2], inout LineStream<GS_OUTPUT_FLUIDSIM> Stream)
{
    GS_OUTPUT_FLUIDSIM Out;
...
float3 Gradient(Texture3D texture3d, GS_OUTPUT_FLUIDSIM input, out bool isBoundary, float minSlope, out bool highEnoughSlope)
{
    ...
    product *= LBD * LBC * LBU * LCD * LCU * LTD * LTC * LTU * CBD * CBU * CTD * CTU * RBD * RBC * RBU * RCD * RCU * RTD * RTC * RTU;
...
float4 PS_ADVECT_MACCORMACK(GS_OUTPUT_FLUIDSIM input) : SV_Target
{
    ...
    ret = Texture_phi.SampleLevel(samLinear, nposTC, 0) + 0.5 * (Texture_phi.SampleLevel(samPointClamp, input.texcoords, 0) -
...
float4 PS_CONFINEMENT(GS_OUTPUT_FLUIDSIM input) : SV_Target
{
    ...
    float4 force;
    force.xyz = timestep * vortConfinementScale * float3(eta.y * omega.z - eta.z * omega.y, eta.z * omega.x - eta.x * omega.z, eta.x * omega.y - eta.y * omega.x);
...
{
    return ((p.x > LBUcorner.x) && (p.x < RTDcorner.x) && (p.y > LBUcorner.y) && (p.y < RTDcorner.y) && (p.z > LBUcorner.z) && (p.z < RTDcorner.z));
}
