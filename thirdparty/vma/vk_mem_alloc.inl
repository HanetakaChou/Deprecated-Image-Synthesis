...
VmaSuballocationList::iterator VmaBlockMetadata_Generic::FindAtOffset(VkDeviceSize offset)
{
    ...
    auto findSuballocation = [&](VmaSuballocationList::iterator begin, VmaSuballocationList::iterator end) -> VmaSuballocationList::iterator
    {
    ...
    auto rfindSuballocation = [&](VmaSuballocationList::reverse_iterator begin, VmaSuballocationList::reverse_iterator end) -> VmaSuballocationList::iterator
    {
        for (auto suballocItem = begin;
             suballocItem != end;
             ++suballocItem)
    ...
    {
        return rfindSuballocation(m_Suballocations.rbegin(), m_Suballocations.rend());
    }
    ...
    VmaAllocation *pAllocation)
{
            ...
            &currRequest))
    {
        ...
        *pAllocation = m_hAllocator->m_AllocationObjectAllocator.Allocate(isUserDataString, isMappingAllowed);
        ...
        (*pAllocation)->InitBlockAllocation(pBlock, currRequest.allocHandle, alignment,
...
VkResult VmaAllocator_T::CreatePool(const VmaPoolCreateInfo *pCreateInfo, VmaPool *pPool)
{
    ...
    {
        typedef struct VMA_VkBaseInStructure
        {
            VkStructureType sType;
            struct VMA_VkBaseInStructure const *pNext;
        } VMA_VkBaseInStructure;
        VMA_ASSERT(((const VMA_VkBaseInStructure *)pCreateInfo->pMemoryAllocateNext)->sType != 0);
    }
